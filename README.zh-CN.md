...
- **Ubuntu / Linux 被控端。** 键鼠注入使用 X11 的 XTEST 扩展，需要在已登录的 Xorg 桌面中以当前桌面用户运行。Wayland 暂不支持；若登录界面提供 **Ubuntu on Xorg**，可注销后选择该会话。缺少显示连接或 XTEST 扩展时会在输入状态和诊断中显示错误。Linux 全局键鼠捕获尚未实现，请将 Linux 设备设为客户端。
