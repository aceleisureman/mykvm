...
- **Ubuntu / Linux 被控端。** 请以已登录的桌面用户运行，并设为**客户端**。X11 会话继续使用 **XTEST**；Wayland 会话新增实验性的 **RemoteDesktop Portal + EIS** 支持，要求桌面的 Portal 后端实现 RemoteDesktop v2 / `ConnectToEIS` 以及 ScreenCast 显示器选择。无需 root、修改 `/dev/uinput` 权限或借助 XWayland。Linux 全局键鼠捕获／服务端模式仍未实现。

### Wayland 授权

客户端启动输入共享时（包括客户端自动启动），请在系统共享对话框中允许**键盘和鼠标控制**，并选择**全部物理显示器**。MyKVM 只利用显示器元数据映射指针，**不会**打开 PipeWire 视频流、采集画面或传输屏幕视频。由于显示器选择由 Portal 提供，桌面仍可能显示屏幕共享指示器。

- 只有授权完成且 EIS 设备和显示器映射就绪后，才会向其它设备广播“可接收输入”。状态／诊断轮询和网络输入包不会触发授权。
- 拒绝、取消、撤销授权、缺少 EIS 支持或显示器映射不明确时，输入保持不可用并显示错误；不会偷偷回退到 XWayland，也不会循环弹出授权。重试时点击**停止 → 启动**；授权不会持久化。
- 停止共享、切换控制端、检测到 QUIC 连接断开时，会释放仍按下的键和鼠标按钮。控制端突然掉线可能需要等待最长约 10 秒的传输空闲超时；撤销授权／移除设备时，由合成器清理虚拟输入状态。
- EIS 设备暂时暂停时，停止接收输入、清空按住状态并丢弃排队命令；只有设备及显示器映射重新就绪，才在原授权内恢复共享，不重放旧按键／鼠标按钮。
- 桌面移除并重建**纯键盘设备**时，等待同一已授权 EIS 连接、同一 seat 上的新键盘恢复，再重新校验未变更的指针映射；即使新键盘先于旧键盘移除而就绪，也会丢弃旧输入。指针／混合设备移除、seat 移除、连接中断或撤销授权仍会失败关闭，需手动重启共享。本机解锁后若桌面始终未恢复键盘，请停止／启动共享重新授权；不会自动续授权。
- 对镜像／重叠显示器布局直接报错，不猜测目标屏幕。更换物理显示器或调整分辨率、缩放、旋转后，请**重启 MyKVM**以刷新原生显示几何，再重新授权。
- 按键沿用现有虚拟键协议并映射到物理 evdev 键码，建议两端使用相同键盘布局；不统一不同布局和输入法行为，也不绕过登录、锁屏或安全注意序列限制。

自动化覆盖及发布前需交互验证的项目见 [Linux 输入验收清单](docs/linux-input-testing.md)。
